...
function showPage(pageNumber) {
    ...
    if (pageNumber === 2) {
        ...
        var rocketLeagueId = document.getElementById('rocket-league-id').value;
        var rank = document.getElementById('rank').value;
        var peakRank = document.getElementById('peak-rank').value;
        var rocketLeagueTracker = document.getElementById('rocket-league-tracker').value;
        if (discord === '' || platform === '' || rocketLeagueId === '' || rank === '' || peakRank === '' || rocketLeagueTracker === '') {
            alert('Veuillez remplir tous les champs avant de passer à la page suivante.');
            return;
        }
        // Vérification si le Tracker Rocket League est un lien valide
        if (!isValidUrl(rocketLeagueTracker)) {
            alert('Le Tracker Rocket League doit être un lien valide commençant par http:// ou https://');
            return;
        }
    }
...
function submitForm() {
    ...
    emailBody += "ID Rocket League : " + rocketLeagueId + "\n";
    emailBody += "Rang actuel : " + rank + "\n";
    emailBody += "Peak rang : " + peakRank + "\n";
    emailBody += "Tracker Rocket League : " + rocketLeagueTracker + "\n";
    ...
    var subject = "Nouvelle candidature Rocket League";
